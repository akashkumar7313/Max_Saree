...
const OtherCollections = () => {
  ...
  const fetchProducts = useCallback(() => {
    const productsRef = firebase.database().ref("OtherCollections");
  ...
  const handleSaveProduct = () => {
    if (selectedImage && productName && productPrice && productColor && productDescription) {
      setUploading(true);
      const storageRef = firebase.storage().ref(`OtherCollections/${selectedImage.name}`);
      ...
      uploadTask.on(
        ...
        () => {
          uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
            firebase.database().ref("OtherCollections").push({
              img: downloadURL,
  ...
  const handleImageDelete = (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      ...
      const productRef = firebase.database().ref(`OtherCollections/${productId}`);
      const imageRef = firebase.database().ref(`OtherCollections`).orderByChild("img").equalTo(products.find(product => product.id === productId).img);
