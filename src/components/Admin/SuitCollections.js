...
const SuitCollections = () => {
  ...
  const handleSaveProduct = () => {
    if (selectedImage && productName && productPrice && productColor && productDescription) {
      setUploading(true);
      const storageRef = firebase.storage().ref(`SuitCollections/${selectedImage.name}`);
      ...
      uploadTask.on(
        ...
        () => {
          uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
            firebase.database().ref("SuitCollections").push({
              img: downloadURL,
  ...
  const handleImageDelete = (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      ...
      const productRef = firebase.database().ref(`SuitCollections/${productId}`);
      const imageRef = firebase.database().ref(`SuitCollections`).orderByChild("img").equalTo(products.find(product => product.id === productId).img);
...
export default SuitCollections;
