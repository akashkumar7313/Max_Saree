...
const Brands = () => {
  ...
  useEffect(() => {
    const imagesRef = firebase.database().ref('Brands');
  ...
  const handleImageDelete = (imageId) => {
    const imageRef = firebase.database().ref(`Brands/${imageId}`);
  ...
  const handleSaveButtonClick = () => {
    if (selectedImage) {
      setUploading(true);
      const storageRef = firebase.storage().ref(`Brands/${selectedImage.name}`);
      ...
      uploadTask.on(
        ...
        () => {
          uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
            firebase.database().ref("Brands").push({
              url: downloadURL,
