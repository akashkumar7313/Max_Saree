...
const Saree = () => {
  ...
  const handleSaveProduct = () => {
    if (selectedImage && productName && productPrice && productColor && productDescription) {
      setUploading(true);
      const storageRef = firebase.storage().ref(`NewArrivals/${selectedImage.name}`);
      ...
      uploadTask.on(
        ...
        () => {
          uploadTask.snapshot.ref.getDownloadURL().then((downloadURL) => {
            firebase.database().ref("NewArrivals").push({
              img: downloadURL,
  ...
  const handleImageDelete = (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      ...
      const productRef = firebase.database().ref(`NewArrivals/${productId}`);
      const imageRef = firebase.database().ref(`NewArrivals`).orderByChild("img").equalTo(products.find(product => product.id === productId).img);
